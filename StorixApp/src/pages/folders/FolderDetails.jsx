...
import { sortFiles, sortFolders } from "../../utils/sortItems";
...
const FolderDetails = () => {
  ...
  const [folderSortBy, setFolderSortBy] = useState("name-asc");
  const [fileSortBy, setFileSortBy] = useState("name-asc");
  ...
  const moveFolderModal = useItemModal();
  const moveFileModal = useItemModal();
  const { folders, files, createFolder, renameFolder: saveFolder, deleteFolder: removeFolder, renameFile: saveFile, deleteFile: removeFile, downloadFile, uploadFiles, moveFolder, moveFile, toggleFavorite } = useFolder();
  ...
  const visibleChildFolders = sortFolders(childFolders.filter(item => item.name.toLowerCase().includes(query)), folderSortBy);
  const visibleFolderFiles = sortFiles(folderFiles.filter(item => item.name.toLowerCase().includes(query)), fileSortBy);
  ...
  folderMenuItems.push(
    { label: "↗ Move", onClick: () => moveFolderModal.openModal(folders.find(item => sameId(item.id, folderMenu.selectedId))) },
    { label: "★ Favorite", onClick: () => toggleFavorite("folder", folderMenu.selectedId) },
  );
  fileMenuItems.push(
    { label: "↗ Move", onClick: () => moveFileModal.openModal(files.find(item => sameId(item.id, fileMenu.selectedId))) },
    { label: "★ Favorite", onClick: () => toggleFavorite("file", fileMenu.selectedId) },
  );
  ...
  return (
    <AppLayout>
      <div className="folder-details">
        ...
        <input ref={fileInputRef} type="file" multiple hidden onChange={event => { uploadFiles(event.target.files, folderId); event.target.value = ""; }} />
        <div className={`folder-content folder-content--${viewMode}`} onDragOver={event => event.preventDefault()} onDrop={event => { event.preventDefault(); uploadFiles(event.dataTransfer.files, folderId); }}>
          <div className="upload-dropzone" onClick={() => fileInputRef.current?.click()} role="button" tabIndex={0} onKeyDown={event => event.key === "Enter" && fileInputRef.current?.click()}>Drop files here or click to upload</div>
          {(childFolders.length > 0 || folderFiles.length > 0) && <div className="folder-sort-bar">
            {childFolders.length > 0 && <SortSelect value={folderSortBy} onChange={setFolderSortBy} label="Sort folders" />}
            {folderFiles.length > 0 && <SortSelect value={fileSortBy} onChange={setFileSortBy} kind="file" label="Sort files" />}
            <ViewToggle value={viewMode} onChange={setViewMode} label="Folder contents view" />
          </div>}
      ...
      <MoveModal isOpen={moveFolderModal.isOpen} item={moveFolderModal.item} folders={folders} itemType="folder" onMove={destination => { moveFolder(moveFolderModal.item.id, destination); moveFolderModal.closeModal(); }} onCancel={moveFolderModal.closeModal} />
      <MoveModal isOpen={moveFileModal.isOpen} item={moveFileModal.item} folders={folders} itemType="file" onMove={destination => { moveFile(moveFileModal.item.id, destination); moveFileModal.closeModal(); }} onCancel={moveFileModal.closeModal} />
